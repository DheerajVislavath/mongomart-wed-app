...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
        ...
        */
        ...
        var categories = this.db.collection("item").aggregate([{$group: {_id: "$category", num: {$sum: 1}}}, {$sort: {"_id": 1}}]).toArray(function(err, categories){
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         */
    if(category == "All"){
            var pageItem = this.db.collection("item").find().sort({"_id": 1}).skip(page).limit(itemsPerPage).toArray(function(
                err, pageItems){
                assert.equal(null, err);   
           ...
           var pageItem = this.db.collection("item").find({"category": category}).sort({"_id": 1}).skip(page).limit(itemsPerPage).toArray(function(
                err, pageItems){
                assert.equal(null, err);
    ...
    this.getNumItems = function(category, callback) {
         ...
         */
         if(category == "All"){
         	var numItems = this.db.collection("item").find({}).count();
         	callback(numItems);
         }
         // TODO Include the following line in the appropriate
         // place within your code to pass the count to the callback.
        else{
        	var numItems = this.db.collection("item").find({"category": category}).count();
        	callback(numItems);
        }
    }
